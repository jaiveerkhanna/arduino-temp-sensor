...
void ds_write1bit(void)
{
    ...
    PORTC &= ~(1 << PC3);
    // turn DDRC into input then output
    DDRC |= (1 << PC3); // backt to low state
    _delay_us(2);
    DDRC &= ~(1 << PC3); //got to high state
    _delay_us(60);
...
unsigned char ds_readbit(void)
{
    ...
    DDRC |= (1 << PC3); // backt to low state
    _delay_us(2);
    DDRC &= ~(1 << PC3); // Release to DS
    _delay_us(10); // sample after 10 micro_seconds
    read = PINC; // sampling here
    _delay_us(50);
    result |= (read & (1 << PC3)); // prepare result, isolate PC3 bit
    return result;
}
